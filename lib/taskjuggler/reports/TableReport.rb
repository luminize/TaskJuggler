...
class TaskJuggler
  ...
  class TableReport < ReportBase
    ...
    def TableReport::defaultColumnTitle(id)
      ...
      # title.
      specials = %w( chart secondly minutely hourly daily weekly monthly quarterly yearly)
  ...
  protected
    ...
    def adjustColumnPeriod(columnDef, tasks = [], scenarios = [])
      ...
      when 'secondly', 'minutely', 'hourly', 'daily', 'weekly', 'monthly', 'quarterly', 'yearly'
    ...
    def generateHeaderCell(columnDef)
      ...
      when 'secondly'
        genCalChartHeader(columnDef, rStart.midnight, rEnd, :sameTimeNextSecond,
                          '%M', '%S')
      when 'minutely'
        genCalChartHeader(columnDef, rStart.midnight, rEnd, :sameTimeNextMinute,
                          '%H', '%M')
      when 'hourly'
  ...
  private
    ...
    def generateTableCell(line, columnDef, query)
      ...
      when 'secondly'
        ...
        sameTimeNextFunc = :sameTimeNextSecond
      when 'minutely'
        start = query.start.midnight
        sameTimeNextFunc = :sameTimeNextMinute
      when 'hourly'
        start = query.start.midnight
